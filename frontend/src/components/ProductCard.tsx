...
const ProductCard = ({ id, image, name, unit, mrp, discountPrice, cartItem }: ProductType) => {
    const { estimatedTime } = useLocation();
    const { addToCart, updateQuantity } = useCart();

    return (
        <>
            <Link to={`/pn/${name}/pid/${id}`}
                className="flex flex-col min-w-full border shadow-md rounded-lg bg-white h-[300px] p-1"
            >
                <div className="p-2 w-full flex justify-center items-center">
                    <img
                        src={image || "https://cdn.grofers.com/cdn-cgi/image/f=auto,fit=scale-down,q=85,metadata=none/da/cms-assets/cms/product/8622bfed-5e2d-415d-8a85-99c5fb7bac04.jpg?ts=1737543569"}
                        ...
                        <div className="flex justify-between items-center mt-2">
                            {mrp ? (
                                <>
                                    <div className="flex flex-col justify-center">
                                        <span className="font-semibold text-xs">₹{discountPrice || 40}</span>
                                        <span className="font-semibold text-xs line-through text-gray-500">₹{mrp || 40}</span>
                                    </div>
                                </>
                            ) : (
                                <>
                            ...
                            {cartItem?.quantity ? (
                                <>
                                    ...
                                    >
                                        <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); updateQuantity(id, cartItem.quantity - 1) }} className="px-2 py-[9px]"><FaMinus className="h-2.5 w-2.5 flex-shrink-0" /></button>
                                        <span>{cartItem?.quantity}</span>
                                        <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); updateQuantity(id, cartItem.quantity + 1) }} className="px-2 py-[9px]"><FaPlus className="h-2.5 w-2.5 flex-shrink-0" /></button>
                                    </div>
                                </>
                            ) : (
                                <>
